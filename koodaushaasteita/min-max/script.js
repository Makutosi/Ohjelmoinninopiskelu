...
そこで、apply を使って配列の要素を個々の引数として展開します。
Math.min.apply(null, [1, 5, 3]);
// Math.min(1, 5, 3) と同じ動作

var max = Math.max.apply(null, arr); 配列 arr の中の最大値を取得
Math.max:
与えられた数値の中で最大値を返す関数です。
例: Math.max(3, 5, 1) は 5 を返します。

Math.max.apply(null, arr):
apply を使い、配列の要素を Math.max に渡して最大値を計算します。

入力チェック: 配列でない、または空配列ならエラーメッセージを返します。
最小値の計算: 配列の中から最小値を取得します。
最大値の計算: 配列の中から最大値を取得します。
結果を文字列として返す: 整形した文字列を返します。
*/